...
class MFRecommender:
    ...
    def load_data(self):
        ...
        try:
            ...
            print(f"📊 Users: {self.num_users}, Movies: {self.num_items}")
    ...
    def load_model(self):
        ...
        self.model = MF(self.num_users, self.num_items).to(self.device)
        print("✅ MF model initialized with random weights")
        self.model.eval()
    ...
    def update_user_ratings(self, user_id, user_ratings):
        ...
        if user_ratings:
            print(f"🔄 Training MF model with {len(user_ratings)} user ratings...")
    ...
    def get_recommendations(self, user_id, top_k=3):
        ...
        print(f"🎯 Generating PERSONALIZED MF recommendations for user {safe_user_id}...")
        ...
        try:
            ...
            print(f"❌ Error in MF recommendations: {e}")
            return self._get_fallback_recommendations()
    
    def _get_fallback_recommendations(self):
        """Fallback recommendations"""
        return [
            {'id': 1, 'title': 'Toy Story', 'genres': 'Animation|Children|Comedy', 'score': 4.8, 'model': 'MF'},
            {'id': 50, 'title': 'The Usual Suspects', 'genres': 'Crime|Mystery|Thriller', 'score': 4.7, 'model': 'MF'},
            {'id': 100, 'title': 'Fargo', 'genres': 'Comedy|Crime|Drama', 'score': 4.6, 'model': 'MF'}
        ]